...
my @combo_composite_link_conflict = ();
...
my $maxLinkLossRate = 0.9;    # prune an edge if its loss rate exceeds 90% (consistent with testbed)
my $effort = 3000;
...
sub run_mimo_routing
{
    ...
    generate_combo_composite_links();
    
    generate_conflict_composite_link($totalCompositeLinks,\@composite_link,\@composite_link_conflict);
    generate_conflict_composite_link($totalComboCompositeLinks,\@combo_composite_link,\@combo_composite_link_conflict);
    # generate_conflict_combo_composite_link();

    findIndepSets_composite($totalCompositeLinks, $effort, $indepMultiFile, \@composite_link_conflict);
    findIndepSets_composite($totalComboCompositeLinks, $effort, $indepMultiComboFile, \@combo_composite_link_conflict);    
    
    my($target_power) = -1;
    my($target_rate) = -1;
    
    my($cmd) = "perl mimo2.pl $mac $randSeed $GAMMA $EPSILON $target_power $target_rate 1 $routingObj $pruningMode $isMultiRate 0 $model";
    print "$cmd\n";
    system($cmd);
...
sub generate_combo_composite_links
{
    my(%seen_combo_composite_link) = ();
    
    foreach my $i ( 0 .. $totalCompositeLinks-1 )
    {
        my($f,$sender_str,$receiver_str) = split ":", $composite_link[$i];
        my(@receiver_lst) = split " ", $receiver_str;
        foreach my $r ( @receiver_lst)
        {
            $seen_combo_composite_link{$f}{$sender_str}{$r} = 1;
        }
    }

    open(comboCompositeLinkFile,">$comboCompositeLinkFile");
    foreach my $f ( keys %seen_combo_composite_link )
    {
        foreach my $sender_str ( keys %{$seen_combo_composite_link{$f}} )
        {
            my(@receiver_lst) = (sort {$a<=>$b} keys %{$seen_combo_composite_link{$f}{$sender_str}});
            my($receiver_str) = join " ", @receiver_lst;
            $combo_composite_link[$totalComboCompositeLinks] = "$f:$sender_str:$receiver_str";
            print comboCompositeLinkFile "$f:$sender_str:$receiver_str\n";
            $totalComboCompositeLinks++;
        }
    }
...
sub enumerate_composite_links
{
	...
	foreach my $i ( 0 .. $totalNodes-1)
	{
	    my(@curr_neighbors) = (keys %{$neighbor_single_rate{$i}});
		...
		# my(@tmp_neighbors) = @curr_neighbors;
		# permute(\@tmp_neighbors);
		# print "tmp_neighbors: "; print (join " ", @tmp_neighbors); print "\n";
		...
		foreach my $j2 ( &enumerate_subset(@complement+0) )
		{
		    ...
		    next if (@sender_lst+0 > @receiver_lst+0);
...
sub generate_conflict_composite_link
{
    ...
    my($curr_totalCompositeLinks, $curr_composite_link, $curr_composite_link_conflict) = @_;
    
    foreach my $i ( 0 .. $curr_totalCompositeLinks-1 )
    {
        my($sender_lst1, $receiver_lst1) = get_composite_link($$curr_composite_link[$i]);
        
        foreach my $j ( $i+1 .. $curr_totalCompositeLinks-1 )
        {
	    $$curr_composite_link_conflict[$i][$j] = 0;
	    $$curr_composite_link_conflict[$j][$i] = 0;

            my($sender_lst2, $receiver_lst2) = get_composite_link($$curr_composite_link[$j]);
		    ...
		    # print "sender1=$sender1 sender2=$sender2\n";
                    ...
                    {
                        $$curr_composite_link_conflict[$i][$j] = 1;
                        $$curr_composite_link_conflict[$j][$i] = 1;
                        last;
                    }
                }
            }
...
# sub generate_conflict_combo_composite_link
# {
#     # composite links ({s_i},{r_j}) interferes with ({s_i'},{r_j'})
#     # iff (i) exists s_i carrier senses s_i' or vice versa
#     #     (ii) exists s_i within interference range of r_j' or vice versa
#     foreach my $i ( 0 .. $totalComboCompositeLinks-1 )
#     {
#         my($sender_lst1, $receiver_lst1) = get_composite_link($combo_composite_link[$i]);
        
#         foreach my $j ( $i+1 .. $totalComboCompositeLinks-1 )
#         {
# 	    $combo_composite_link_conflict[$i][$j] = 0;
# 	    $combo_composite_link_conflict[$j][$i] = 0;

#             my($sender_lst2, $receiver_lst2) = get_composite_link($combo_composite_link[$j]);
...
#                     if (distance($sender1, $sender2) < $CarrierSenseRange)
#                     {
#                         $combo_composite_link_conflict[$i][$j] = 1;
#                         $combo_composite_link_conflict[$j][$i] = 1;
#                         last;
#                     }
#                 }
#             }
