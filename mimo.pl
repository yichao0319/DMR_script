...
my $use_indeptSet;
my $model;
my $MIMO_header_size = 42; # XXX: Yi-Chao please change it to the current header size
...
sub generate_flow_conservation
{
    ...
    foreach my $f ( 0 .. $nflows-1 )
    {
		...
		foreach my $power ( keys %allPowers )
		{
			...
			{
			    my($flow, $sender_str, $receiver_str) = split ":", $composite_link[$vl];
			    my($NA) = &get_NA_cl($sender_str, $receiver_str);
			    my(@receiver_lst) = split " ", $receiver_str;
			    my($ratio) = $num_antenna{$i}/$NA;
    ...
    foreach my $power ( keys %allPowers )
    {
		...
		my($num_sender_antennas) = 0;
                ...
                foreach my $s (@sender_lst)
                {
                    $num_sender_antennas += $num_antenna{$s};
                }
				...
				# deliver to a receiver only when all senders deliver successfully
                                foreach my $s ( @sender_lst )
                                {
				    foreach my $ss ( 0 .. $num_antenna{$s}-1 )
				    {
...
sub generate_interference_constraints
{
	...
	my($NA) = &get_NA_cl($sender_str,$receiver_str);
		...
		my($efficiency) = &cal_efficiency($NA);
    ...
    foreach my $scale ( 1  )  # ( 1, 1.1, 1.2, 1.5 )
    {
        ...
        foreach my $power ( sort {$a <=> $b} keys %allPowers )
        {
			...
			print SrcAppFile "$f $flow_src $vl1 $TL{$power}{$rate}{$f}{$vl1} $ival $min_interval\n";
			    ...
			    my(@sender_lst2) = split " ", $sender_str2;
                            my(@receiver_lst2) = split " ", $receiver_str2;
			    my($NA) = &get_NA_cl($sender_str2, $receiver_str2);
                            my($redundancy);
