...
my $source_MIMO = 1;
my $dest_MIMO = 1;
my $NUM_ANTENNAS_MIMO = 1;
my %num_antennas = (); 
my $ETX_IMPROVEMENT_THRESH =  0.5;
...
if ($mac == 0){
    if($default_rate == 11){
	$TR = 415.3;	# 452.91472221866; #      # communication range
    }elsif($default_rate == 1){
	$TR = 1012.3;	# 1027.799111221291;
...
my %composite_link_conflict = ();   # yichao
...
my $maxLinkLossRate = 0.5;    # prune an edge if its loss rate exceeds 90% (consistent with testbed)
my $effort = 3000;
...
sub run_mimo_routing
{
    ...
    print STDERR "elapsed enumerate_composite_links time: ".$elapsed."\n";

    generate_conflict_composite_link();
    my @t2 = gettimeofday();
    $elapsed = tv_interval (\@t1, \@t2);
    print STDERR "elapsed generate_conflict_composite_link time: ".$elapsed."\n";

    # findIndepSets_composite($totalCompositeLinks, max($effort*$nflows/5, 1), $indepMultiFile, \@composite_link_conflict, \@composite_link_single_antenna_sender);
    findIndepSets_composite($totalCompositeLinks, max($effort*$nflows/2, 100), $indepMultiFile, \%composite_link_conflict, \@composite_link_single_antenna_sender);   # yichao
    my @t3 = gettimeofday();
    $elapsed = tv_interval (\@t2, \@t3);
    print STDERR "elapsed findIndepSets_composite time: ".$elapsed."\n";


    my($target_power) = -1;
    my($target_rate) = $default_rate;
    print "target_rate=$target_rate\n";
    my($cmd) = "perl mimo3.pl $mac $randSeed $GAMMA $EPSILON $target_power $target_rate 1 $routingObj $pruningMode $isMultiRate 0 $model";
    print "$cmd\n";
    system($cmd);
    my @t4 = gettimeofday();
    $elapsed = tv_interval (\@t3, \@t4);
    print STDERR "elapsed mimo3.pl time: ".$elapsed."\n";

    
    # remove a receiver if it doesn't generate outgoing traffic
    $cmd = "perl revise-credit3.pl";
    print "$cmd\n";
    system($cmd);
    my @t5 = gettimeofday();
    $elapsed = tv_interval (\@t4, \@t5);
    print STDERR "elapsed revise-credit3.pl time: ".$elapsed."\n";
    ...
    # generate MIMO nodes
    $cmd = "perl post-process-mimo3.pl";
    print "$cmd\n";
    system($cmd);
    my @t6 = gettimeofday();
    $elapsed = tv_interval (\@t5, \@t6);
    print STDERR "elapsed post-process-mimo3.pl time: ".$elapsed."\n";
    ...
    my @t7 = gettimeofday();
    $elapsed = tv_interval (\@t6, \@t7);
    print STDERR "elapsed waste time: ".$elapsed."\n";
...
#            system("perl process-stat.pl -t $transport -s $sim_time -c \"$comment\" -m $MAX_THROUGHPUT topo.stat >> summary-multibatch.qualnet.$scheme.rl");
    ...
    my @t0 = gettimeofday();
	# system("./qualnet-$scheme lp-nrl.$scheme.config > /dev/null 2>&1");
...
sub enumerate_composite_links
{
	...
	foreach my $i ( 0 .. $totalNodes-1)
	{
	    ...
	    next if ($cost{$i}{$dest} > $cost{$src}{$dest}-$ETX_IMPROVEMENT_THRESH && $i != $src);
	    ...
	    {
		next if ($cost{$neigh}{$dest} > $cost{$src}{$dest}-$ETX_IMPROVEMENT_THRESH && $i != $src);
		...
		foreach my $r ( @curr_neighbors )
		{
			...
			if (distance($r, $s) > $TR || 
			    ...
			    $cost{$r}{$dest} > $cost{$s}{$dest} - $ETX_IMPROVEMENT_THRESH)
			{
			    $validate = 0;
			}
		    }
		...
		my(@sender_lst) = (sort {$a<=>$b} @sender_lst);
		my(@receiver_lst) = (sort {$a<=>$b} @receiver_lst);
		my($sender_str) = join " ", @sender_lst;
		my($receiver_str) = join " ", @receiver_lst;

		next if (exists $seen{$f}{$sender_str}{$receiver_str});

		$seen{$f}{$sender_str}{$receiver_str} = 1;

		# enumerate virtual links for each number of antenna combinations at the senders 
		my(@antenna_config_lst) = "$f:$sender_str:$receiver_str:";

		foreach my $s ( @sender_lst )
		{
		    my(@new_antenna_config_lst) = ();
		    
		    foreach my $i ( 1 .. $num_antennas{$s} )
		    {
			foreach my $antenna_config ( @antenna_config_lst )
			{
			    my($new_antenna_config) = join " ", $antenna_config, $i;
			    push @new_antenna_config_lst, $new_antenna_config;
			}
		    }
		    @antenna_config_lst = @new_antenna_config_lst;
		}
		
		print "antenna_config_lst: ";
		print (join "\n", @antenna_config_lst);
		print "\n";

		foreach my $config ( @antenna_config_lst )
		{
		    my(@lst) = split ":", $config;
		    my(@sender_antennas_lst) = split " ", $lst[$#lst];
		    my($total_sender_antennas) = 0;
		    
		    foreach my $ss_na ( @sender_antennas_lst )
		    {
			$total_sender_antennas += $ss_na;
		    }
...
sub get_composite_link
{
    ...
    my($flow, $sender_lst_str, $receiver_lst_str, $antenna_lst_str) = split ":", $link;
...
sub generate_conflict_composite_link
{
    ...
    print STDERR "generate_conflict_composite_link: $totalCompositeLinks links\n";

    foreach my $i ( 0 .. $totalCompositeLinks-1 )
    {
        my($sender_lst1, $receiver_lst1) = get_composite_link($i);

        my $tmp1 = length($sender_lst1);
        my $tmp2 = length($receiver_lst1);
        # print STDERR "generate_conflict_composite_link: $tmp1 tx, $tmp2 rx\n";
        
        foreach my $j ( $i+1 .. $totalCompositeLinks-1 )
        {
	        # $composite_link_conflict[$i][$j] = 0;
	        # $composite_link_conflict[$j][$i] = 0;
            $composite_link_conflict{"$i,$j"} = 0;   # yichao
            # $composite_link_conflict{"$j,$i"} = 0;   # yichao
            

            my($sender_lst2, $receiver_lst2) = get_composite_link($j);
	        # print "i=$i j=$j "; print (join " ",@$sender_lst1); print ":"; print (join " ",@$sender_lst2); print "\n";
	    
            # print STDERR "size " . keys( %composite_link_conflict ) . ".\n";
		    ...
		    # print "sender1=$sender1 sender2=$sender2 ";
                    if (distance($sender1, $sender2) < $CarrierSenseRange)
                    {
                        ...
                        # $composite_link_conflict{"$i,$j"} = 1;   # yichao
                        # $composite_link_conflict{"$j,$i"} = 1;   # yichao
                        delete $composite_link_conflict{"$i,$j"};
                        last;
                    }
                }
                # start yichao
                # if($composite_link_conflict[$i][$j] == 1) {
                if(!defined($composite_link_conflict{"$i,$j"}) ) {
                    last;
                }
                # end yichao
            }
            ...
            next if (!defined($composite_link_conflict{"$i,$j"}));   # yichao

            # check (ii)
            # print STDERR "check 2\n";
