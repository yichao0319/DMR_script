...
my $use_indeptSet;
my $model;
my $MIMO_header_size = 58; # 74 XXX: Yi-Chao please change it to the current header size
...
my $preamble;
my $rate = $target_rate;
# my $efficiency = cal_efficiency($MIMO_header_size); # xxx: please parse the right header size
...
my @t00 = gettimeofday();
my $elapsed = tv_interval (\@t000, \@t00);
print STDERR "elapsed init mimo3 time: ".$elapsed."\n";

read_loss();
read_traffic();
read_etx() if ($pruningMode);
read_composite_link();   # mimo xxx
read_indept_composite(); # if ($use_indeptSet); # mimo
read_ackroute() if ($include_ack_overhead != 0);
read_antenna();
my @t01 = gettimeofday();
$elapsed = tv_interval (\@t00, \@t01);
print STDERR "elapsed read time: ".$elapsed."\n";


map_var2id();
my @t02 = gettimeofday();
$elapsed = tv_interval (\@t01, \@t02);
print STDERR "elapsed map_var2id time: ".$elapsed."\n";
...
my @t0 = gettimeofday();
$elapsed = tv_interval (\@t02, \@t0);
print STDERR "elapsed generate_obj time: ".$elapsed."\n";
...
print STDERR "elapsed generate_flow_conservation time: ".$elapsed."\n";

# print lpFile "flow bound\n";
generate_flow_bound(); 
my @t2 = gettimeofday();
$elapsed = tv_interval (\@t1, \@t2);
print STDERR "elapsed generate_flow_bound time: ".$elapsed."\n";

# print lpFile "interference\n";
generate_interference_constraints() if ($use_indeptSet); # only modify interference constraints
my @t3 = gettimeofday();
$elapsed = tv_interval (\@t2, \@t3);
print STDERR "elapsed generate_interference_constraints time: ".$elapsed."\n";

generate_bounds();     
my @t4 = gettimeofday();
$elapsed = tv_interval (\@t3, \@t4);
print STDERR "elapsed generate_bounds time: ".$elapsed."\n";
...
if ($use_indeptSet) # use indeptSet always have one pass and need to process_output right after solving LP
{
    ...
    my @t5 = gettimeofday();
    $elapsed = tv_interval (\@t4, \@t5);
    print STDERR "elapsed cplex_solve.sh time: ".$elapsed."\n";

    &process_output();
    my @t6 = gettimeofday();
    $elapsed = tv_interval (\@t5, \@t6);
    print STDERR "elapsed process_output time: ".$elapsed."\n";
...
elsif ($process_output)
{
    ...
    my @t5 = gettimeofday();
    $elapsed = tv_interval (\@t4, \@t5);
    print STDERR "elapsed convert_matlab_to_cplex_result time: ".$elapsed."\n";
...
sub map_var2id
{
	...
	foreach my $power ( keys %allPowers )
	{
		...
		next if ($target_rate != -1 && $target_rate != $rate);
                ...
                my($f, $sender_str, $receiver_str, $antenna_str) = split ":", $composite_link[$vl];
...
sub get_composite_link
{
    ...
    my($flow, $sender_lst_str, $receiver_lst_str, $antenna_str) = split ":", $link;
    my(@sender_lst) = split " ", $sender_lst_str;
    my(@receiver_lst) = split " ", $receiver_lst_str;
    return(\@sender_lst, \@receiver_lst);
}
...
sub read_composite_link
{
	...
	$composite_link[$totalCompositeLinks] = "$flow:$sender_str:$receiver_str:$antenna_str";
...
sub generate_flow_conservation
{
        ...
        foreach my $rate ( keys %allRates )
        {
            ...
            foreach my $f ( 0 .. $nflows-1 )
            {
                ...
                foreach my $curr_dest ( @flow_dest )
                {
                    ...
                    {
                        my($flow, $sender_str, $receiver_str, $antenna_str) = split ":", $composite_link[$vl];
                        my(@receiver_lst) = split " ", $receiver_str; # ??? 2/18
                        # print lpFile "debug vl $vl $sender_str:$receiver_str\n";
                        foreach my $r ( @receiver_lst )
                        {
    ...
    foreach my $f ( 0 .. $nflows-1 )
    {
        ...
        foreach my $curr_dest ( @flow_dest )
        {
            ...
            {
		my($to_print) = 0;
		# print "i=$i flow_src=$flow_src curr_dest=$curr_dest\n";
                next if ($flow_src == $i);
                next if ($curr_dest == $i);
		...
		foreach my $power ( keys %allPowers )
		{
			...
			{
			    my($flow, $sender_str, $receiver_str, $antenna_str) = split ":", $composite_link[$vl];
			    my($NA_total) = &get_num_antennas_cl($sender_str, $receiver_str, $antenna_str);
			    my($NA_i) = &get_num_antennas_node($i, $sender_str, $antenna_str);
			    my(@receiver_lst) = split " ", $receiver_str;
			    my($ratio) = $NA_i/$NA_total;
    ...
    foreach my $power ( keys %allPowers )
    {
		...
		my($num_sender_antennas) = 0;
                
                foreach my $na (@antenna_lst)
                {
                    $num_sender_antennas += $na;
                }
                ...
                foreach my $curr_dest ( @flow_dest )
                {
                    ...
                    {
                        my $vloss = 1;
                        my $num_receiver_antennas = 0;
                        
                        foreach my $k ( 0 .. $#receiver_lst )
                        {
                            if (($j & (1<<$k)) > 0)
                            {
                                my($curr_receiver) = $receiver_lst[$k];
				$num_receiver_antennas += $num_antenna{$curr_receiver};
				my($delivery_rate) = 1;
				...
				# deliver to a receiver only when all senders deliver successfully
				    ...
				    foreach my $ss ( 0 .. $a-1 )
				    {
					$delivery_rate *= (1-$loss{$power}{$rate}{$s}{$curr_receiver});
				    }
                                }
				...
				$vloss *= (1-$delivery_rate);
                        ...
                        printf lpFile "+%.9f TL(%d,%d,%d,%d) ",(1-$vloss)*&min($num_sender_antennas,$num_receiver_antennas),$power,$rate,$f,$vl if ($gen_lp);
                        $str = "TL($power,$rate,$f,$vl)";
...
sub get_num_nodes_cl
{
    my($sender_str, $receiver_str) = @_;
    my(@receiver_lst) = split " ", $receiver_str;
    my(@sender_lst) = split " ", $sender_str;
    return(&min(@receiver_lst+0, @sender_lst+0));
}
...
sub get_num_antennas_cl
{
    my($sender_str, $receiver_str, $antenna_str) = @_;
    my(@receiver_lst) = split " ", $receiver_str;
    my(@sender_lst) = split " ", $sender_str;
    my(@antenna_lst) = split " ", $antenna_str;
    my($num_sender_antennas) = 0;
    ...
    foreach my $a ( @antenna_lst )
    {
	$num_sender_antennas += $a;
    }
    
    foreach my $rr ( @receiver_lst )
    {
	$num_receiver_antennas+= $num_antenna{$rr};
    }

    return(&min($num_sender_antennas, $num_receiver_antennas));
}
...
sub generate_interference_constraints
{
	...
	my(@sender_lst) = split " ", $sender_str;
	my(@receiver_lst) = split " ", $receiver_str;
	my($num_antennas) = &get_num_antennas_cl($sender_str,$receiver_str,$antenna_str);
		...
		my($efficiency) = &cal_efficiency($num_antennas);
            ...
            foreach my $f ( 0 .. $nflows-1 )
            {
                ...
                foreach my $curr_dest ( @flow_dest )
                {
                    foreach my $vl ( 0 .. $totalCompositeLinks-1 )
                    {
                        ...
                        foreach my $r ( @receiver_lst )
                        {
                            print lpFile "0 <= Y($power,$rate,$f,$curr_dest,$vl,$r)\n" if ($gen_lp);
                            $str = "Y($power,$rate,$f,$curr_dest,$vl,$r)";
                            $LB{$var2id{$str}} = 0;
                            # $UB{$var2id{$str}} = $Ymax;
                        }
...
#            printf "new max info: $power,$rate,$f,$i,$j => %f\n", $max_info{$power}{$rate}{$f}{$i}{$j}; 
    ...
    foreach my $power ( sort {$a <=> $b} keys %allPowers )
    {
        foreach my $rate ( sort {$a <=> $b} keys %allRates )
        {
            ...
            {
		my($f,$sender_str,$receiver_str,$antenna_str) = split ":", $composite_link[$vl1];
                my(@sender_lst) = split " ", $sender_str;
                my($flow_demand,$flow_src,@flow_dest) = split " ", $flow[$f];
                my(@receiver_lst) = split " ", $receiver_str;

                if ((!exists $TL{$power}{$rate}{$f}{$vl1} || $TL{$power}{$rate}{$f}{$vl1} < $fwdThresh) && $flow_src != $sender_lst[0])
                {
		    ...
		    if (!exists $delivery_rate{$power}{$rate}{$vl1}{$r})
		    {
			$recv = 0;
		    }
		    else
		    {
			$recv = $TL{$power}{$rate}{$f}{$vl1}*$delivery_rate{$power}{$rate}{$vl1}{$r};
		    }
                    my($total_traffic) = 0;
		    foreach my $vl2 ( keys %{$sender2compositeLink{$f}{$r}} )
		    {
			$total_traffic += $TL{$power}{$rate}{$f}{$vl2} if (exists $TL{$power}{$rate}{$f}{$vl2});
		    }
		    foreach my $vl2 ( keys %{$sender2compositeLink{$f}{$r}} )
		    {
			if ($recv > 0 && $total_traffic > 0)
			{
			    ...
			    my(@sender_lst2) = split " ", $sender_str2;
                            my(@receiver_lst2) = split " ", $receiver_str2;
			    my($num_sender_antennas) = 0;
                            my($num_receiver_antennas) = 0;
			    
			    foreach my $ss ( @sender_lst2 )
                            {
                                $num_sender_antennas += $num_antenna{$ss};
                            }

                            foreach my $rr ( @receiver_lst2 )
                            {
                                $num_receiver_antennas += $num_antenna{$rr};
			    }

			    # my($ratio) = $num_sender_antennas/$num_receiver_antennas;
			    $weight{$f}{$power}{$rate}{$vl1}{$vl2}{$r} = $max_info{$power}{$rate}{$f}{$vl1}{$r}/$recv*$TL{$power}{$rate}{$f}{$vl2}/$total_traffic; # $ratio;
    ...
    foreach my $scale ( 1  )  # ( 1, 1.1, 1.2, 1.5 )
    {
        ...
        foreach my $power ( sort {$a <=> $b} keys %allPowers )
        {
			...
			print SrcAppFile "$f $flow_src $vl1 $TL{$power}{$rate}{$f}{$vl1} $ival $min_interval\n";
			    ...
			    my(@sender_lst2) = split " ", $sender_str2;
                            my(@receiver_lst2) = split " ", $receiver_str2;
			    my($NA) = &get_num_antennas_cl($sender_str2, $receiver_str2, $antenna_str2);
                            my($redundancy);
                            ...
                            {
				my($NA) = &min($NA);
                                $redundancy = $TL{$power}{$rate}{$f}{$vl2}*$NA/$total_max_info_composite{$f}{$vl2};
                            }
...
sub cal_efficiency
{
   # return(1);
   my($num_header) = @_;
   # print "cal_efficiency: num_header = $num_header\n";
   my $pktsize = ($payload+20+8+28+22/8) + (28+22/8+$MIMO_header_size)*$num_header; # data pkt size +  header size * num_header
   ...
   $DIFS = 2*$slot + $SIFS;
   $EP = $payload*8/$rate;
   $H = $preamble*($num_header+1) + ($pktsize - $payload)*8/$rate;
   # should not include CW/2                                                                                                     
   print "cal_efficiency: EP=$EP payload=$payload rate=$rate ", $payload*8/$rate," ", $EP/($H + $EP + $DIFS + $CW/2*$slot)," ",$EP/($H + $EP + $DIFS),"\n";
   return($EP/($H + $EP + $DIFS));
}
